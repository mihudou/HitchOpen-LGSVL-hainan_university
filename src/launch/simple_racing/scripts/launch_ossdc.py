...
uuids = json.load(open("src/launch/simple_racing/params/uuids.json", "r"))
parser = argparse.ArgumentParser()
parser.add_argument("--env", help="Environment file to use", default="simple_racing.env", required=False)
parser.add_argument("--num_cars", help="Number of cars to spawn", default=1, required=False)
args = parser.parse_args()

# Read the mapping config file
env = Env()
env.read_env(f"src/launch/simple_racing/params/{args.env}")
