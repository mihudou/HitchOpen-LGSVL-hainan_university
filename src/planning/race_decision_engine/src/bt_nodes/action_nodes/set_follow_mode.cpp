...
BT::PortsList SetFollowMode::providedPorts()
{
  return {
...
BT::NodeStatus SetFollowMode::tick()
{
  config().blackboard->set<uint8_t>("strategy_type", race_msgs::msg::StrategyType::FOLLOW_MODE);
  RCLCPP_INFO_THROTTLE(
    logger_, clock_,
    config().blackboard->get<const std::shared_ptr<ros_parameters::Params>>(
      "rde_params")->non_critical_log_time_period_ms, "Setting to follow mode");
