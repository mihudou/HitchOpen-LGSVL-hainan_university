...
BT::NodeStatus SetPitLaneSpeed::tick()
{
  config().blackboard->set<std::string>("speed_type", "pit_lane");
  ...
  RCLCPP_INFO_THROTTLE(
    ...
    config().blackboard->get<const std::shared_ptr<ros_parameters::Params>>(
      "rde_params")->non_critical_log_time_period_ms, "Pit lane speed");
