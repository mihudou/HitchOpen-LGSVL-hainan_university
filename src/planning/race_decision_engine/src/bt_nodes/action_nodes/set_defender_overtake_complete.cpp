...
SetDefenderOvertakeComplete::SetDefenderOvertakeComplete(
  const std::string & name, const BT::NodeConfiguration & config,
  const rclcpp::Logger & logger, rclcpp::Clock & clock)
: BT::SyncActionNode(name, config), logger_(logger), clock_(clock)
{}

BT::PortsList SetDefenderOvertakeComplete::providedPorts()
{
  return {
    BT::InputPort<const std::shared_ptr<ros_parameters::Params>>(
      "rde_params"),
    BT::InputPort<bool>("target_value"),
    BT::OutputPort<bool>("defender_overtake_complete"),
  };
}

BT::NodeStatus SetDefenderOvertakeComplete::tick()
{
  config().blackboard->set<bool>(
    "defender_overtake_complete",
    getInput<bool>("target_value").value());
  // RCLCPP_WARN_THROTTLE(
  //   logger_, clock_, config().blackboard->get<const std::shared_ptr<ros_parameters::Params>>(
  //     "rde_params")->non_critical_log_time_period_ms, "Setting defender overtake complete to %d",
  //   getInput<bool>("target_value").value());
