...
SetDefenderOvertakenGap::SetDefenderOvertakenGap(
  const std::string & name, const BT::NodeConfiguration & config,
  const rclcpp::Logger & logger, rclcpp::Clock & clock)
: BT::SyncActionNode(name, config), logger_(logger), clock_(clock)
{}

BT::PortsList SetDefenderOvertakenGap::providedPorts()
{
  return {
...
BT::NodeStatus SetDefenderOvertakenGap::tick()
{
  config().blackboard->set<std::string>("gap_type", "defender_overtaken_gap");
  RCLCPP_INFO_THROTTLE(
    logger_, clock_,
    config().blackboard->get<const std::shared_ptr<ros_parameters::Params>>(
      "rde_params")->non_critical_log_time_period_ms, "Defender Overtaken Gap");
