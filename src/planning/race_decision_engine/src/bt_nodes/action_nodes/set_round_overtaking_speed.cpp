...
SetRoundOvertakingSpeed::SetRoundOvertakingSpeed(
  const std::string & name, const BT::NodeConfiguration & config,
  const rclcpp::Logger & logger, rclcpp::Clock & clock)
: BT::SyncActionNode(name, config), logger_(logger), clock_(clock)
{}

BT::PortsList SetRoundOvertakingSpeed::providedPorts()
{
  return {
...
BT::NodeStatus SetRoundOvertakingSpeed::tick()
{
  config().blackboard->set<std::string>("speed_type", "round_overtaking");
  ...
  RCLCPP_INFO_THROTTLE(
    ...
    config().blackboard->get<const std::shared_ptr<ros_parameters::Params>>(
      "rde_params")->non_critical_log_time_period_ms, "Round overtaking speed");
