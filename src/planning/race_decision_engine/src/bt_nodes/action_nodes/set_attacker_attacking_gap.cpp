...
SetAttackerAttackingGap::SetAttackerAttackingGap(
  const std::string & name, const BT::NodeConfiguration & config,
  const rclcpp::Logger & logger, rclcpp::Clock & clock)
: BT::SyncActionNode(name, config), logger_(logger), clock_(clock)
{}

BT::PortsList SetAttackerAttackingGap::providedPorts()
{
  return {
...
BT::NodeStatus SetAttackerAttackingGap::tick()
{
  config().blackboard->set<std::string>("gap_type", "attacker_attacking_gap");
  RCLCPP_INFO_THROTTLE(
    logger_, clock_,
    config().blackboard->get<const std::shared_ptr<ros_parameters::Params>>(
      "rde_params")->non_critical_log_time_period_ms, "Attacker Attacking Gap");
