...
BT::NodeStatus SetRoundSpeed::tick()
{
  config().blackboard->set<std::string>("speed_type", "round");
  ...
  RCLCPP_INFO_THROTTLE(
    ...
    config().blackboard->get<const std::shared_ptr<ros_parameters::Params>>(
      "rde_params")->non_critical_log_time_period_ms, "Round speed");
