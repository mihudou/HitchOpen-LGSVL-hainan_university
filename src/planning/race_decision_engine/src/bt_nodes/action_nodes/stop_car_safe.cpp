...
BT::NodeStatus StopCarSafe::tick()
{
  config().blackboard->set<std::string>("speed_type", "stop");
  config().blackboard->set<uint8_t>("stop_type", race_msgs::msg::StopType::STOP_TYPE_SAFE);
  ...
  RCLCPP_INFO_THROTTLE(
    ...
    config().blackboard->get<const std::shared_ptr<ros_parameters::Params>>(
      "rde_params")->non_critical_log_time_period_ms, "Safe stop");
