...
FillSecRivalCarInfo::FillSecRivalCarInfo(
...
BT::PortsList FillSecRivalCarInfo::providedPorts()
{
  return {
    ...
    BT::InputPort<const common::RivalCar>("sec_rival_car"),
    BT::OutputPort<common::RivalCar>("previous_sec_rival_car"),
    BT::BidirectionalPort<common::RivalCar>("current_sec_rival_car")
  };
}

BT::NodeStatus FillSecRivalCarInfo::tick()
{
  bool previous_rival_car_exists = true;
  common::RivalCar previous_rival_car;
  try {
    previous_rival_car = config().blackboard->get<common::RivalCar>("current_sec_rival_car");
    config().blackboard->set<common::RivalCar>(
      "previous_sec_rival_car", previous_rival_car);
  } catch (const std::runtime_error & ex) {
    previous_rival_car_exists = false;
    previous_rival_car = common::CreateFalseRivalCar();
  }
  common::RivalCar rival_car = config().blackboard->get<common::RivalCar>("sec_rival_car");
  if (previous_rival_car_exists) {
    ...
    RCLCPP_INFO_THROTTLE(
      logger_, clock_, config().blackboard->get<const std::shared_ptr<ros_parameters::Params>>(
        "rde_params")->non_critical_log_time_period_ms, "Secondary rival car latched onto line %d",
  ...
  if (rival_car.exists) {
    ...
    RCLCPP_INFO_THROTTLE(
      ...
      "Secondary rival car tracked on line %d with front gap %f and back gap %f",
      static_cast<int>(rival_car.current_ttl), rival_car.front_gap, rival_car.back_gap);
  }

  config().blackboard->set<common::RivalCar>("current_sec_rival_car", rival_car);
