...
BT::PortsList IfInPitBox::providedPorts()
{
  return {
...
BT::NodeStatus IfInPitBox::tick()
{
  if (config().blackboard->get<race::ttl::TtlIndex>(
    ...
    race::ttl::TrackLocation::PIT_BOX)
  {
    RCLCPP_INFO_THROTTLE(
      logger_, clock_,
      config().blackboard->get<const std::shared_ptr<ros_parameters::Params>>(
        "rde_params")->non_critical_log_time_period_ms, "In Pit Box");
