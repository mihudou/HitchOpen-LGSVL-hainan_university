...
IfRivalCarFrontGapGreaterThan::IfRivalCarFrontGapGreaterThan(
  const std::string & name,
  const BT::NodeConfiguration & config)
: BT::ConditionNode(name, config)
{}

BT::PortsList IfRivalCarFrontGapGreaterThan::providedPorts()
{
  return {
...
BT::NodeStatus IfRivalCarFrontGapGreaterThan::tick()
{
  bool rival_car_exists =
    config().blackboard->get<const common::RivalCar>("current_rival_car").exists;
  double rival_car_front_gap =
    config().blackboard->get<const common::RivalCar>("current_rival_car").front_gap;
  ...
  if (!rival_car_exists) {
    ...
    if (!thresh_to_compare_against) {
      ...
      if (rival_car_front_gap > thresh_to_compare_against.value()) {
