...
BT::PortsList IfYellowFlag::providedPorts()
{
  return {
...
BT::NodeStatus IfYellowFlag::tick()
{
  if (config().blackboard->get<const race::Flags>(
      "current_flags").has_yellow_flag())
  {
    RCLCPP_INFO_THROTTLE(
      logger_, clock_,
      config().blackboard->get<const std::shared_ptr<ros_parameters::Params>>(
        "rde_params")->non_critical_log_time_period_ms, "Yellow Flag");
