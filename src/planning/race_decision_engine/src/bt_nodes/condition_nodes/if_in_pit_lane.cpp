...
BT::PortsList IfInPitLane::providedPorts()
{
  return {
...
BT::NodeStatus IfInPitLane::tick()
{
  if (config().blackboard->get<race::ttl::TtlIndex>(
    ...
    race::ttl::TrackLocation::PIT_LANE)
  {
    RCLCPP_INFO_THROTTLE(
      logger_, clock_,
      config().blackboard->get<const std::shared_ptr<ros_parameters::Params>>(
        "rde_params")->non_critical_log_time_period_ms, "In Pit Lane");
