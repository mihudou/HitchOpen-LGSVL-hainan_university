...
IfRaceControlTimeout::IfRaceControlTimeout(
...
BT::PortsList IfRaceControlTimeout::providedPorts()
{
  return {
...
BT::NodeStatus IfRaceControlTimeout::tick()
{
  if (common::CheckTimeout(
      ...
      config().blackboard->get<const std::shared_ptr<ros_parameters::Params>>(
        "rde_params")->timeout.manual.race_control_timeout,
      config().blackboard->get<const std::shared_ptr<ros_parameters::Params>>(
        "rde_params")->timeout.autonomous.race_control_timeout,
      config().blackboard->get<const common::RdeInputs::ConstSharedPtr>(
        "rde_inputs")->race_control_diff))
  {
    RCLCPP_WARN_THROTTLE(
      logger_, clock_,
      config().blackboard->get<const std::shared_ptr<ros_parameters::Params>>(
        "rde_params")->non_critical_log_time_period_ms, "Race Control timeout in %d with %f",
      config().blackboard->get<const std::shared_ptr<ros_parameters::Params>>(
        "rde_params")->manual_overrides.auto_enabled,
      config().blackboard->get<const common::RdeInputs::ConstSharedPtr>(
        "rde_inputs")->race_control_diff);
