...
IfInPitEntrance::IfInPitEntrance(
...
BT::PortsList IfInPitEntrance::providedPorts()
{
  return {
...
BT::NodeStatus IfInPitEntrance::tick()
{
  if (config().blackboard->get<race::ttl::TtlIndex>(
    ...
    config().blackboard->get<const common::CarState>(
      ...
      config().blackboard->get<race::ttl::TtlIndex>(
        "pit_ttl_index")).waypoints.at(
      ttl_tree_->find_closest_waypoint_index(
        config().blackboard->get<race::ttl::TtlIndex>(
          "pit_ttl_index"), config().blackboard->get<const common::CarState>(
          "current_car_state").pos)).region ==
    race::ttl::TrackLocation::PIT_ENTRANCE)
  {
    RCLCPP_INFO_THROTTLE(
      logger_, clock_,
      config().blackboard->get<const std::shared_ptr<ros_parameters::Params>>(
        "rde_params")->non_critical_log_time_period_ms, "In Pit Entrance");
