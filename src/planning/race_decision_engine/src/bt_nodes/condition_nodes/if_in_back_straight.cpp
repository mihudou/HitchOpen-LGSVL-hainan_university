...
IfInBackStraight::IfInBackStraight(
...
BT::PortsList IfInBackStraight::providedPorts()
{
  return {
...
BT::NodeStatus IfInBackStraight::tick()
{
  ...
  {
    double back_start_percentage =
      config().blackboard->get<const std::shared_ptr<ros_parameters::Params>>(
      "rde_params")->back_straight.start_percentage;
    double back_end_percentage =
      config().blackboard->get<const std::shared_ptr<ros_parameters::Params>>(
      "rde_params")->back_straight.end_percentage;
    double current_percentage = config().blackboard->get<const common::CarState>(
      "current_car_state").lap_percentage;
    if (back_start_percentage <= back_end_percentage) {
      if (current_percentage >= back_start_percentage &&
        current_percentage <= back_end_percentage)
      {
        RCLCPP_INFO_THROTTLE(
          logger_, clock_,
          config().blackboard->get<const std::shared_ptr<ros_parameters::Params>>(
            "rde_params")->non_critical_log_time_period_ms, "In Back Straight");
