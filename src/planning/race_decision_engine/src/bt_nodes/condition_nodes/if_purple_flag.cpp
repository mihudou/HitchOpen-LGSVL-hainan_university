...
BT::PortsList IfPurpleFlag::providedPorts()
{
  return {
...
BT::NodeStatus IfPurpleFlag::tick()
{
  if (config().blackboard->get<const race::Flags>(
      "current_flags").has_purple_flag())
  {
    RCLCPP_ERROR_THROTTLE(
      logger_, clock_,
      config().blackboard->get<const std::shared_ptr<ros_parameters::Params>>(
        "rde_params")->non_critical_log_time_period_ms, "Purple Flag");
