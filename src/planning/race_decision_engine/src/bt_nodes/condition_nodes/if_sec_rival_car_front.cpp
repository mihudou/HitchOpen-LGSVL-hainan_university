...
{
IfSecRivalCarFront::IfSecRivalCarFront(
  const std::string & name,
  const BT::NodeConfiguration & config)
: BT::ConditionNode(name, config) {}      // IfSecRivalCarFront

BT::PortsList IfSecRivalCarFront::providedPorts()
{
  return {BT::InputPort<const common::RivalCar>("current_secondary_rival_car")};
}  // providedPorts

BT::NodeStatus IfSecRivalCarFront::tick()
{
