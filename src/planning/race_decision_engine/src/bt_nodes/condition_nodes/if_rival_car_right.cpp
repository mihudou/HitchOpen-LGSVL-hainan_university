...
IfRivalCarRight::IfRivalCarRight(
  const std::string & name,
  const BT::NodeConfiguration & config)
: BT::ConditionNode(name, config)
{}

BT::PortsList IfRivalCarRight::providedPorts()
{
  return {
    BT::InputPort<const ttl::TtlIndex>("right_ttl_index"),
    BT::InputPort<const common::RivalCar>("current_rival_car")
  };
}

BT::NodeStatus IfRivalCarRight::tick()
{
  ...
  if (!rival_car_exists) {
    ...
    if (rival_car_idx == config().blackboard->get<ttl::TtlIndex>("right_ttl_index")) {
