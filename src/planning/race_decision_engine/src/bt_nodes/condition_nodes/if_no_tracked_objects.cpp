...
IfNoTrackedObjects::IfNoTrackedObjects(
  const std::string & name,
  const BT::NodeConfiguration & config)
: BT::ConditionNode(name, config)
{}

BT::PortsList IfNoTrackedObjects::providedPorts()
{
  return {
...
BT::NodeStatus IfNoTrackedObjects::tick()
{
  if (config().blackboard->get<const common::RdeInputs::ConstSharedPtr>(
      "rde_inputs")->opp_car_detections_msg->objects.empty())
  {
