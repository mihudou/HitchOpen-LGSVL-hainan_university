...
BT::PortsList IfEKillFlag::providedPorts()
{
  return {
...
BT::NodeStatus IfEKillFlag::tick()
{
  if (config().blackboard->get<const race::Flags>(
      "current_flags").has_ekill_flag())
  {
    RCLCPP_INFO_THROTTLE(
      logger_, clock_,
      config().blackboard->get<const std::shared_ptr<ros_parameters::Params>>(
        "rde_params")->non_critical_log_time_period_ms, "EKill Flag");
