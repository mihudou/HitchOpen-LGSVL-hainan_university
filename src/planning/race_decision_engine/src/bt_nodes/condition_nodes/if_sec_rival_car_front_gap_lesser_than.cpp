...
{
IfSecRivalCarFrontGapLesserThan::IfSecRivalCarFrontGapLesserThan(
  const std::string & name, const BT::NodeConfiguration & config)
: BT::ConditionNode(name, config) {}      // IfSecRivalCarFrontGapLesserThan

BT::PortsList IfSecRivalCarFrontGapLesserThan::providedPorts()
{
...
BT::NodeStatus IfSecRivalCarFrontGapLesserThan::tick()
{
  ...
  if (!secondary_rival_car_exists) {
    ...
    if (!thresh_to_compare_against) {
      ...
      if (secondary_rival_car_front_gap < thresh_to_compare_against.value()) {
