...
BT::PortsList IfInPitRoad::providedPorts()
{
  return {
...
BT::NodeStatus IfInPitRoad::tick()
{
  if (config().blackboard->get<race::ttl::TtlIndex>(
    ...
    race::ttl::TrackLocation::PIT_ROAD)
  {
    RCLCPP_INFO_THROTTLE(
      logger_, clock_,
      config().blackboard->get<const std::shared_ptr<ros_parameters::Params>>(
        "rde_params")->non_critical_log_time_period_ms, "In Pit Road");
