...
IfCrossedPassingSf::IfCrossedPassingSf(
  const std::string & name,
  const BT::NodeConfiguration & config)
: BT::ConditionNode(name, config)
{}

BT::PortsList IfCrossedPassingSf::providedPorts()
{
  return {
...
BT::NodeStatus IfCrossedPassingSf::tick()
{
  try {
    if (config().blackboard->get<const common::CarState>("previous_car_state").lap_distance <
      config().blackboard->get<const std::shared_ptr<ros_parameters::Params>>(
        "rde_params")->passing_sf_threshold &&
